...
int main(){
    ...
    do{ //loop infinitely so program never exits
        int numCustomers = getLoyaltyDatabase(customers); 
        customerIndex = runLoyaltyProgram(customers, &numCustomers);
        if(customerIndex == -2){
            return -2;
        }
        ...
        customers[customerIndex].setNumItemsInOrder(0);
        placeOrder(&customers[customerIndex]);
        
        if(isLoyaltyMember){
            int availablePts = customers[customerIndex].getNumberLoyaltyPoints();
            int numPts = customers[customerIndex].calculateCost();
            bool usingLoyalty = pointSystemPrompt(&customers[customerIndex]);

            float customerMoney = 0;
            if(!usingLoyalty){
                availablePts += numPts;
                customers[customerIndex].displayOrder();
                customers[customerIndex].setNumberLoyaltyPoints(availablePts);
                calculateChange(&customers[customerIndex]);
                cout<< "Have a nice day, and thank you for choosing Meaty Meat Man's Market!" << endl << endl <<endl << endl;
            }
            else{
                useLoyalty(&customers[customerIndex], numCustomers, availablePts);
                customers[customerIndex].displayOrder();
                calculateChange(&customers[customerIndex]);
                cout<< "Have a nice day, and thank you for choosing Meaty Meat Man's Market!" << endl << endl <<endl << endl;
            }
            ...
            customers[numCustomers].displayOrder();
            calculateChange(&customers[numCustomers]);
            cout<< "Have a nice day, and thank you for choosing Meaty Meat Man's Market!" << endl << endl <<endl << endl;
            customers[numCustomers].setNumItemsInOrder(0);
        }
        overwriteLoyaltyDatabase(customers, numCustomers);
